...
class MNIST_loader:
    ...
    def _data_loader(self, data, idx):
        """
        Creates an iterable on the data
        Arguments:
    ...
    def train_data(self, client = None):
        ...
        '''
        if client != None:
            return self._data_loader([self._x_train, self._y_train], self.clients[f"client_{client}"]["train"])
        else:
            all_samples_train = []
            for client in self.clients:
                all_samples_train += self.clients[client]["train"]
            return self._data_loader([self._x_train, self._y_train], all_samples_train)
    ...
    def val_data(self, client = None):
        ...
        '''
        if client != None:
            return self._data_loader([self._x_train, self._y_train], self.clients[f"client_{client}"]["val"])
        else:
            all_samples_val = []
            for client in self.clients:
                all_samples_val += self.clients[client]["val"]
            return self._data_loader([self._x_train, self._y_train], all_samples_val)
    ...
    def test_data(self):
        ...
        return self._data_loader([self._x_test, self._y_test], _idx)